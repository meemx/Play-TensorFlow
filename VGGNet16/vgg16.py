...
class VggNet16:
    def __init__(self, imgs, weights=None, sess=None):
        self.imgs = imgs
        self.convlayers()  # 卷积部分
        self.fc_layers()  # 全连接部分
        self.probs = tf.nn.softmax(self.fc3l)  # Softmax输出
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc1') as scope:
            ...
            fc1w = tf.Variable(tf.compat.v1.truncated_normal([shape, 4096],
                                                             dtype=tf.float32,
                                                             stddev=1e-1), name='weights')
        ...
        with tf.name_scope('fc2') as scope:
            fc2w = tf.Variable(tf.compat.v1.truncated_normal([4096, 4096],
                                                             dtype=tf.float32,
                                                             stddev=1e-1), name='weights')
        ...
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.compat.v1.truncated_normal([4096, 1000],
                                                             dtype=tf.float32,
                                                             stddev=1e-1), name='weights')
    ...
    def load_weights(self, weight_file, sess):
        weights = np.load(weight_file)  # 读取Numpy数据文件
        keys = sorted(weights.keys())  # 排序后按照正常顺序排列
...
if __name__ == '__main__':
    ...
    preds = np.argsort(-prob)[0:5]  # 按下标排序
    for p in preds:
